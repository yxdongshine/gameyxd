	...
	public void invoke(InnerMessage msg, IConnect con) {
		...
		try {
			...
			if (clientIConnect != null && (serverType > 0 || msg.getCommand() < 255)) {// 发到客户端
				...
				// log.info(buffer.limit() + ":发给客户端 !!!!!!!!!!" + Integer.toHexString(msg.getCommand()) + "::" + (data.length + NetConst.CODE_LENGTH));
			} else {
				GameMessage<InnerMessage> task = (GameMessage<InnerMessage>) commond.get(msg.getCommand());
				if (task != null) {
					task.doMessage(msg, con);
				} else {
					log.error(Integer.toHexString(msg.getCommand()) + ":::GateInnerClient不能处理的消息:::" + msg.getClientSessionId());
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
