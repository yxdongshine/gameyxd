 ...
 * Reason: TODO (职责: 1:转发到客户端 2:处理内部服务的心跳逻辑和断线,等). <br/>
...
public class InnerServerControl implements IBeanInvoke<InnerMessage> {
	...
	public void invoke(InnerMessage msg, IConnect con) {
		...
		try {
			// log.error(Integer.toHexString(msg.getCommand()) + ":::GateInnerServer不能处理的消息:::" +msg.getClientSessionId());
			...
			// 内转外
			if (clientIConnect != null && (serverType > 0 || msg.getCommand() < 255)) {// 发到客户端
				// 封装成iobuffer
				...
				// log.info(buffer.limit() + ":内部服发给客户端 !!!!!!!!!!" + Integer.toHexString(msg.getCommand()) + "::" + (data.length + NetConst.CODE_LENGTH));
			} else {
