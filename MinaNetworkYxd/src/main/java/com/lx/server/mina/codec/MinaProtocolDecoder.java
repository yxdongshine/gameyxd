...
public class MinaProtocolDecoder extends CumulativeProtocolDecoder {
	private static final Logger log = LoggerFactory
			.getLogger(MinaProtocolDecoder.class);
	private MinaProtocolCodec codec;

	public MinaProtocolDecoder(MinaProtocolCodec codec) {
		this.codec = codec;
	}
	...
	protected boolean doDecode(IoSession session, IoBuffer in,
		...
		if (in.remaining() > 4) {
			...
			in.get(data);
			BaseMessage.MinaMessage msg = null;
			try {
