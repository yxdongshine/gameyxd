...
public class AStar2 {
	...
	private final SpaceInfo data; // 地图掩码
	// ====================================
	// Member Variables
	// ====================================
	 ...
	 */
	public AStar2(SpaceInfo data, int p_maxTry) {
		this.data = data;
	 ...
	 */
	public List<Position3D> find(Position3D start, Position3D end, boolean bOffset) {
		startTime = System.currentTimeMillis();
		int p_startX = data.getCol(start.getX());
		int p_startY = data.getRow(start.getZ());
		int p_endX = data.getCol(end.getX());
		int p_endY = data.getRow(end.getZ());
		...
		int score; // 节点路径评分F值
		int step = 0;
		while (this.m_openCount > 0 && (step <= m_maxTry || m_maxTry == -1)) {
...
//			}
			...
			if (currNoteX == p_endX && currNoteY == p_endY) {
				return this.getPath(p_startX, p_startY, currId, bOffset);
			}
	...
	/**
	 * @private 将(新加入开放别表或修改了路径评分的)节点向前移动
	 */
	...
	/**
	 * @private 将(取出开启列表中路径评分最低的节点后从队尾移到最前的)节点向后移动
	 */
	...
	/**
	 * @private 获取某节点的周围节点，排除不能通过和已在关闭列表中的
	 */
	private List<int[]> getArounds(int p_x, int p_y) {
		...
		checkX = p_x + 1;
		checkY = p_y;
		boolean canRightBottom = isBalk || isPass(checkX, checkY);
		...
		checkX = p_x;
		checkY = p_y - 1;
		boolean canLeftBottom = isBalk || isPass(checkX, checkY);
		...
		checkX = p_x - 1;
		checkY = p_y;
		boolean canLeftTop = isBalk || isPass(checkX, checkY);
		...
		checkX = p_x;
		checkY = p_y + 1;
		boolean canRightTop = isBalk || isPass(checkX, checkY);
		...
		checkX = p_x + 1;
		checkY = p_y + 1;
		boolean canRight = isBalk || isPass(checkX, checkY);
		...
		checkX = p_x + 1;
		checkY = p_y - 1;
		boolean canDown = isBalk || isPass(checkX, checkY);
		...
		checkX = p_x - 1;
		checkY = p_y - 1;
		boolean canLeft = isBalk || isPass(checkX, checkY);
		...
		checkX = p_x - 1;
		checkY = p_y + 1;
		boolean canUp = isBalk || isPass(checkX, checkY);
	 ...
	 */
	private List<Position3D> getPath(int p_startX, int p_startY, int p_id, boolean bOffset) {
		// Object[] arr = new Object[]{};
		List<Position3D> list = new ArrayList<Position3D>();
		// int noteX = this.m_xList.get(p_id);
		// int noteY = this.m_yList.get(p_id);
		Point point = this.m_List.get(p_id);
		while (point.x != p_startX || point.y != p_startY) {
			// // arr.unshift([noteX, noteY]); //加到头部
			BlockData bData = data.blocks[point.y][point.x];
			float tmp = 0;
			if (bOffset) {
				int r = ServerRandomUtils.nextInt(100);
				int rTmp = (r % 2 == 0) ? data.mapCellWidth : -data.mapCellWidth;
				tmp = ServerRandomUtils.randomFloat((float)rTmp / 2);
			}
			list.add(0, new Position3D(data.getGridX(point.x) + tmp, bData.y, data.getGridZ(point.y) + tmp));
		...
		list.add(0, new Position3D(data.getGridX(p_startX) + tmp, bData.y, data.getGridZ(p_startY) + tmp));
		this.destroyLists();
		// log.error("花费多少时间寻路:::"+(System.currentTimeMillis() - startTime)+"::"+JSON.toJSONString(list));
		return list;
	}
	
	/**
	 * @private 获取某ID节点在开放列表中的索引(从1开始)
	 */
