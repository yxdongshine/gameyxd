...
public class MessageSend {
	...
	public static void sendToWorld(Message msg, ServerPlayer sp) {
		...
		Integer head = MinaClient.protocolMap.get(msg.getDescriptorForType().getName());
		if (head != null) {
	...
	public static void sendToGate(Message msg, ServerPlayer sp) {
		IConnect con = ConnectorManage.getConnectFromList(sp.getGateServerTypeId());
		if (con == null) {
			log.error("没有log服务器的连接::" + msg.getDescriptorForType().getName());
			return;
		}
